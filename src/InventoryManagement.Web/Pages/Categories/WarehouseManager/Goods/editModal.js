var abp = abp || {};
abp.modals.goodsEditModal = function () {
    var initModal = function (publicApi, args) {
        ...
        service.getNotSelectedUnit($('#Id').val()).done(function (data) {
            data.items.forEach(function (e) {
                unitLeftDataTable.row.add(e).draw();
            });
        });

        service.getSelectedUnit($('#Id').val()).done(function (data) {
            data.items.forEach(function (e) {
                unitRightDataTable.row.add(e).draw();
                if ($('#ViewModel_UnitOfGood').val() == '') {
                    $('#ViewModel_UnitOfGood').val(e.id);
                } else {
                    var unitOfGoodVal = $('#ViewModel_UnitOfGood').val() + ',' + e.id;
